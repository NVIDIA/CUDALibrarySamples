...
run_example(const std::vector<std::vector<char>> &data, size_t warmup_iteration_count, size_t total_iteration_count)
{
  ...
  BatchDataCPU compressed_data_cpu(snappy::MaxCompressedLength(chunk_size), chunk_count);
  ...
  for (size_t i = 0; i < chunk_count; ++i)
  {
    ...
    {
      throw std::runtime_error("Snappy CPU failed to compress chunk " + std::to_string(i) + ".");
    }
  }

  // compute compression ratio
  size_t comp_bytes =
    std::accumulate(compressed_data_cpu.sizes(), compressed_data_cpu.sizes() + chunk_count, size_t(0));
  ...
  nvcompStatus_t status =
    nvcompBatchedSnappyDecompressGetRequiredAlignments(decompress_opts, &decompression_alignment_reqs);
