...
run_example(const std::vector<std::vector<char>> &data, size_t warmup_iteration_count, size_t total_iteration_count)
{
  ...
  nvcompStatus_t status =
    nvcompBatchedLZ4CompressGetRequiredAlignments(nvcompBatchedLZ4Opts, &compression_alignment_reqs);
  ...
  status = nvcompBatchedLZ4CompressGetMaxOutputChunkSize(chunk_size, nvcompBatchedLZ4Opts, &max_out_bytes);
  if (status != nvcompSuccess)
  {
