...
__global__ void compare(const uint8_t *ref, const uint8_t *val, int *invalid, size_t n)
{
  size_t i = (size_t)blockIdx.x * blockDim.x + threadIdx.x;
...
int main(int argc, char **argv)
{
  ...
  const DecompressionConfig decomp_config = compressor.configure_decompression(comp_config);
