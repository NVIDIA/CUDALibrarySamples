...
)
{
  ...
  for (size_t i = 0; i < chunk_count; ++i)
  {
    ...
    { //libdeflate
      ...
      if (len == 0)
      {
        throw std::runtime_error("libdeflate_deflate_compress failed to compress chunk " + std::to_string(i) + ".");
      }
      actual_len = len;
    }
    else if (algo == 1)
    { //zlib::compress2
      ...
      if (compression_level < Z_NO_COMPRESSION || compression_level > Z_BEST_COMPRESSION)
      {
        throw std::runtime_error(
          ...
          ". Supported range: " + std::to_string(Z_NO_COMPRESSION) + " - " + std::to_string(Z_BEST_COMPRESSION)
        );
      }
      int ret = compress2(
        (uint8_t *)compressed_data_cpu.ptrs()[i],
        &len,
  ...
  nvcompStatus_t status =
    nvcompBatchedDeflateDecompressGetRequiredAlignments(decompress_opts, &decompression_alignment_reqs);
...
int main(int argc, char *argv[])
{
  ...
  {
    std::cerr << "Must choose an algorithm via '-a <algo>'. '<algo>' can be 0, 1, or 2. (0 libdeflate, 1 "
                 "zlib_compress2, 2 zlib_deflate)"
              << std::endl;
    return 1;
  }
