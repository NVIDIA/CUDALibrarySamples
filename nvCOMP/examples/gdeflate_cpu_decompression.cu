...
run_example(const std::vector<std::vector<char>> &data, size_t warmup_iteration_count, size_t total_iteration_count)
{
  ...
  static_assert(
    chunk_size <= nvcompGdeflateCompressionMaxAllowedChunkSize,
  ...
  nvcompStatus_t status =
    nvcompBatchedGdeflateCompressGetRequiredAlignments(nvcompBatchedGdeflateOpts, &compression_alignment_reqs);
  ...
  status = nvcompBatchedGdeflateCompressGetMaxOutputChunkSize(chunk_size, nvcompBatchedGdeflateOpts, &max_out_bytes);
  if (status != nvcompSuccess)
  {
