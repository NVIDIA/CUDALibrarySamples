...
)
{
  ...
  static_assert(
    chunk_size <= nvcompDeflateCompressionMaxAllowedChunkSize,
  ...
  nvcompStatus_t status =
    nvcompBatchedDeflateCompressGetRequiredAlignments(nvcompBatchedDeflateOpts, &compression_alignment_reqs);
  ...
  status = nvcompBatchedDeflateCompressGetMaxOutputChunkSize(chunk_size, nvcompBatchedDeflateOpts, &max_out_bytes);
  if (status != nvcompSuccess)
  {
  ...
  for (size_t i = 0; i < chunk_count; ++i)
  {
    if (algo == 0)
    {
      ...
      {
        throw std::runtime_error("libdeflate CPU failed to decompress chunk " + std::to_string(i) + ".");
      }
    }
    else if (algo == 1)
    {
...
int main(int argc, char *argv[])
{
  ...
  if (argc < 5)
  {
    std::cerr << "Must choose an algorithm via '-a <algo>', and must specify at least one file via '-f <file>'."
              << std::endl;
    return 1;
  }
  else if (algo < 0 || algo > 1)
  {
    std::cerr << "Must choose an algorithm via '-a <algo>'. '<algo>' can be 0 or 1. (0 libdeflate, 1 zlib_inflate)"
              << std::endl;
    return 1;
  }
