...
run_example(const std::vector<std::vector<char>> &data, size_t warmup_iteration_count, size_t total_iteration_count)
{
  ...
  static_assert(
    chunk_size <= snappy::kBlockSize,
    "Chunk size must be less than the constant specified in the Snappy library"
  );
  static_assert(
    chunk_size <= nvcompSnappyCompressionMaxAllowedChunkSize,
  ...
  nvcompStatus_t status =
    nvcompBatchedSnappyCompressGetRequiredAlignments(nvcompBatchedSnappyOpts, &compression_alignment_reqs);
  ...
  status = nvcompBatchedSnappyCompressGetMaxOutputChunkSize(chunk_size, nvcompBatchedSnappyOpts, &max_out_bytes);
  if (status != nvcompSuccess)
  {
  ...
  for (size_t i = 0; i < chunk_count; ++i)
  {
    ...
    result = result && snappy::RawUncompress(
                         static_cast<const char *>(compressed_data_cpu.ptrs()[i]),
                         compressed_data_cpu.sizes()[i],
    ...
    {
      throw std::runtime_error("Snappy CPU failed to decompress chunk " + std::to_string(i) + ".");
    }
  }
