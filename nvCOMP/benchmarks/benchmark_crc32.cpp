...
std::string human_readable_byte_size(size_t size) { return human_readable_size(size) + "B"; }
...
args_type parse_args(int argc, char *argv[])
{
  ...
  args.min_buf_size = 32 * Ki;
  args.max_buf_size = 1 * Gi; // Cap the sweep; larger single buffers make the config search dominate runtime
  args.min_buf_count = 1;
  args.max_buf_count = 64 * Ki; // High enough to exercise WarpKernel
  args.step_shift = 3; // x8 steps: reach 1 GiB buffers while keeping the sweep quick
...
double BenchmarkContext::run_single_benchmark(size_t buf_size, size_t buf_count)
{
  ...
  CUDA_CHECK(cudaMemcpyAsync(chunk_ptrs_d_, chunk_ptrs_h_, buf_count * sizeof(void *), cudaMemcpyHostToDevice));
  CUDA_CHECK(cudaMemcpyAsync(chunk_sizes_d_, chunk_sizes_h_, buf_count * sizeof(size_t), cudaMemcpyHostToDevice));
  ...
  try
  {
    ...
    for (size_t size_idx = 0; size_idx < table.buf_sizes.size(); ++size_idx)
    {
      ...
      for (size_t count_idx = 0; count_idx < table.buf_counts.size(); ++count_idx)
      {
        ...
        table.contents[size_idx][count_idx] = ctx.run_single_benchmark(buf_size, buf_count);
      }
    }
