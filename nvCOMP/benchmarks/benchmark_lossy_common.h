...
{
  const double error_bound = std::is_same<T, half>::value
                               ? static_cast<double>(__hdiv(__double2half(delta), __double2half(2.0)))
                               : delta / 2.0;
  for (size_t j = 0; j < num_elements; j++)
  {
    ...
    if (is_nan || is_inf)
    {
      if ((is_nan && !std::isnan(reconstructed_comp)) || (is_inf && !std::isinf(reconstructed_comp)))
      {
        throw nvcomp::NVCompException(
          ...
          "Lossy bitcomp compression produced invalid special value"
        );
      }
      continue;
    }
    ...
    if (diff > error_bound)
    {
      std::cout << std::setprecision(std::numeric_limits<T>::max_digits10) << "Original: " << original_comp
                << ", Quantized: " << reconstructed_comp << ", Delta: " << delta << ", Diff: " << diff << std::endl;
      throw nvcomp::NVCompException(nvcompErrorInvalidValue, "Lossy bitcomp compression error exceeds allowed delta");
    }
  }
}

inline bitcompDataType_t getLossyDataType(const int fp_bits)
{
  switch (fp_bits)
  {
    case 16:
...
inline void verifyLossyCompression(
  const void *input,
  const void *output,
