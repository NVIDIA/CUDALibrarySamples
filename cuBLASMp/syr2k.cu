...
template <typename T>
static bool check_syr2k_result(
...
{
    static_assert(std::is_same_v<T, double>, "syr2k sample reference check supports double only");
    ...
    if (rank == 0)
    {
        ...
        passed = allclose_device("syr2k", full_C_result, full_C_result_lld, full_C_ref, full_C_ref_lld, n, n, stream);
...
static Result run_syr2k(const Options& opts, ncclComm_t comm)
{
    Result result;
    ...
    CUBLASMP_CHECK(cublasMpSyr2k_bufferSize(
        handle,
    ...
    const bool passed = !opts.check_result || check_syr2k_result(
                                                  handle,
                                                  comm,
                                                  stream,
                                                  rank,
...
int main(int argc, char* argv[])
{
    ...
    const Result result = comm.collective_launch([&](ncclComm_t nccl_comm) { return run_syr2k(opts, nccl_comm); });

    if (comm.is_root() && result.status == CUBLASMP_STATUS_SUCCESS)
    {
        printf("Duration: %lf GFlops: %lf\n", result.elapsed, (2.0 * opts.n * opts.n * opts.k * 1e-9) / result.elapsed);
    }
