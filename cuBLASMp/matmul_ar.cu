...
static Result run_matmul_ar(const Options& opts, ncclComm_t comm)
{
    ...
    if (!ta || tb)
    {
        throw std::runtime_error("The matmul_ar sample supports only transA=t/c and transB=n");
    }
    ...
    // AR only supports a transposed/conjugate-transposed A and non-transposed B.
