...
{
    static_assert(std::is_same_v<T, double>, "syrk sample reference check supports double only");
    ...
    if (rank == 0)
    {
        ...
        passed = allclose_device("syrk", full_C_result, full_C_result_lld, full_C_ref, full_C_ref_lld, n, n, stream);
...
static Result run_syrk(const Options& opts, ncclComm_t comm)
{
    Result result;
    ...
    const bool passed = !opts.check_result || check_syrk_result(
                                                  handle,
                                                  comm,
                                                  stream,
                                                  rank,
...
int main(int argc, char* argv[])
{
    ...
    const Result result = comm.collective_launch([&](ncclComm_t nccl_comm) { return run_syrk(opts, nccl_comm); });
