...
{
    static_assert(std::is_same_v<T, double>, "trmm sample reference check supports double only");
    ...
    gather_matrix(
        ...
        &full_A_lld);
    gather_matrix(
        mp_handle, comm, stream, m, n, d_B, ib, jb, descB, grid, nprow, npcol, myprow, mypcol, &full_B, &full_B_lld);
    gather_matrix(
        mp_handle,
        comm,
        stream,
    ...
    full_C_ref_lld = std::max<int64_t>(1, cublasMpNumroc(m, m, myprow, rsrc, nprow));
    const int64_t full_C_ref_cols = std::max<int64_t>(1, cublasMpNumroc(n, n, mypcol, csrc, npcol));
    CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&full_C_ref), full_C_ref_lld * full_C_ref_cols * sizeof(T)));
    ...
    if (rank == 0)
    {
        ...
        passed = allclose_device("trmm", full_C_result, full_C_result_lld, full_C_ref, full_C_ref_lld, m, n, stream);
...
static Result run_trmm(const Options& opts, ncclComm_t comm)
{
    Result result;
    ...
    generate_random_matrix(m, m, h_A.data(), mbA, nbA, ia, ja, llda, nprow, npcol, myprow, mypcol, rank);
    generate_random_matrix(m, n, h_B.data(), mbB, nbB, ib, jb, lldb, nprow, npcol, myprow, mypcol, rank);
    ...
    CUBLASMP_CHECK(cublasMpTrmm_bufferSize(
        handle,
        CUBLAS_SIDE_LEFT,
    ...
    const bool passed = !opts.check_result || check_trmm_result(
                                                  handle,
                                                  comm,
                                                  stream,
                                                  rank,
...
int main(int argc, char* argv[])
{
    ...
    const Result result = comm.collective_launch([&](ncclComm_t nccl_comm) { return run_trmm(opts, nccl_comm); });

    if (comm.is_root() && result.status == CUBLASMP_STATUS_SUCCESS)
    {
        printf("Duration: %lf GFlops: %lf\n", result.elapsed, (1.0 * opts.m * opts.m * opts.n * 1e-9) / result.elapsed);
    }
