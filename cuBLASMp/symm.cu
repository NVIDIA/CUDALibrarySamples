...
{
    static_assert(std::is_same_v<T, double>, "symm sample reference check supports double only");
    ...
    if (rank == 0)
    {
        ...
        passed = allclose_device("symm", full_C_result, full_C_result_lld, full_C_ref, full_C_ref_lld, m, n, stream);
...
static Result run_symm(const Options& opts, ncclComm_t comm)
{
    Result result;
    ...
    CUBLASMP_CHECK(cublasMpSymm_bufferSize(
        handle,
        CUBLAS_SIDE_LEFT,
    ...
    const bool passed = !opts.check_result || check_symm_result(
                                                  handle,
                                                  comm,
                                                  stream,
                                                  rank,
...
int main(int argc, char* argv[])
{
    ...
    const Result result = comm.collective_launch([&](ncclComm_t nccl_comm) { return run_symm(opts, nccl_comm); });

    if (comm.is_root() && result.status == CUBLASMP_STATUS_SUCCESS)
    {
        printf("Duration: %lf GFlops: %lf\n", result.elapsed, (2.0 * opts.m * opts.m * opts.n * 1e-9) / result.elapsed);
    }
