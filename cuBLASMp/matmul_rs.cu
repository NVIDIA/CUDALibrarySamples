...
static Result run_matmul_rs(const Options& opts, ncclComm_t comm)
{
    Result result;
    ...
    generate_random_matrix(
        ...
        tb ? rank : 0,
        rank);
    generate_random_matrix(m, n, h_D.data(), loc_d_m, loc_d_n, 1, 1, loc_d_m, 1, nranks, 0, rank, rank);
...
template <typename TypeA, typename TypeB, typename TypeD>
static Result run_matmul_rs_workers(Comm& comm, const Options& opts)
{
    return comm.collective_launch(
        [&](ncclComm_t nccl_comm) { return run_matmul_rs<TypeA, TypeB, TypeD>(opts, nccl_comm); });
}
