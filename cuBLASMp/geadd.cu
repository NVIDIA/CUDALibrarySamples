...
static Result run_geadd(const Options& opts, ncclComm_t comm)
{
    Result result;
    ...
    generate_random_matrix(m, n, h_A.data(), mbA, nbA, ia, ja, llda, nprow, npcol, myprow, mypcol, rank);
...
int main(int argc, char* argv[])
{
    ...
    const Result result = comm.collective_launch([&](ncclComm_t nccl_comm) { return run_geadd(opts, nccl_comm); });
