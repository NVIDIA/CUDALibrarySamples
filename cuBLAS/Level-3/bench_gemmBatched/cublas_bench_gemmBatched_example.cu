...
int main(const int argc, const char *argv[]) {
    ...
    std::vector<std::vector<data_type>> A_array(batch_count, std::vector<data_type>(lda*k));
    std::vector<std::vector<data_type>> B_array(batch_count, std::vector<data_type>(ldb*n));
    ...
    CUBLAS_CHECK(cublasSgemmBatched(cublasH, transa, transb, m, n, k, &alpha, d_A_array, lda,
                                    d_B_array, ldb, &beta, d_C_array, ldc, batch_count));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    cudaDeviceSynchronize();
    end = std::chrono::system_clock::now();
    printf("cublas<X>gemmBatched time (microseconds): %ld\n",
