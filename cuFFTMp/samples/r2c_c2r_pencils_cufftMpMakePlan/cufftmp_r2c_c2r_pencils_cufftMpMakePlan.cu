 ...
 * - inverse transform
 * 
 * This sample also illustrates the use of the new API `cufftMpMakePlanDecomposition`
 */
...
void run_r2c_c2r_pencils(size_t nx, size_t ny, size_t nz, float* cpu_data, Box3D box_real, Box3D box_complex, const int rank, const int size, MPI_Comm comm) {
    ...
    // // Describe the data distribution and make the plan
    ...
    CUFFT_CHECK(cufftMpMakePlanDecomposition(plan_r2c, 3, n, box_real.lower, box_real.upper, box_real.strides, box_complex.lower, box_complex.upper, box_complex.strides, CUFFT_R2C, &comm, CUFFT_COMM_MPI, &workspace));
    CUFFT_CHECK(cufftMpMakePlanDecomposition(plan_c2r, 3, n, box_real.lower, box_real.upper, box_real.strides, box_complex.lower, box_complex.upper, box_complex.strides, CUFFT_C2R, &comm, CUFFT_COMM_MPI, &workspace));
