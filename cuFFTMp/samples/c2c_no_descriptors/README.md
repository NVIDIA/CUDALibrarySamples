...
This sample shows a simple example of complex-to-complex distributed FFT computation, by performing a FORWARD FFT -> element-wise transform -> INVERSE FFT, without multi-GPU descriptors. It illustrates the meaning of the `CUFFT_XT_INPLACE` and `CUFFT_XT_INPLACE_SHUFFLED` data distribution and the use of the BoxIterator's.
