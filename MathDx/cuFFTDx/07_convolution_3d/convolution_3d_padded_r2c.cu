...
template<int Batches, class FFTXPartial, int SignalLengthX, class FFTYPartial, int SignalLengthY, class FFTZPartial, int SignalLengthZ, class LoadFunctor, class FilterFunctor, class StoreFunctor, typename IOType>
auto cufftdx_3d_convolution_padded(IOType* input, IOType* output, cudaStream_t stream) {
    ...
    static constexpr bool is_z_power_of_2 = is_power_of_2(fft_size_z);
    static constexpr auto z_real_mode     = is_z_power_of_2 ? cufftdx::real_mode::folded : cufftdx::real_mode::normal;
    ...
    using inter_type = cufftdx::complex<precision>;

    static constexpr unsigned int z_dim           = FFTZ::output_length;
    ...
    static constexpr unsigned int x_batches = z_dim * fft_size_y;
    static constexpr unsigned int y_batches = z_dim * signal_length_x;
    ...
    inter_type* inter;
    const auto  flat_inter_size_bytes = flat_batch_size * sizeof(inter_type);
    ...
    auto kernel_z_front = fft_kernel<FFTZ, io_z_front, LoadFunctor, id_op, io_type, inter_type>;
    set_kernel_shared_size(kernel_z_front, z_max_bytes);

    // No transform ops for middle dimension
    auto kernel_y_front = fft_kernel<FFTY, io_y_front, id_op, id_op, inter_type>;
    ...
    // No transform ops for middle dimension
    auto kernel_y_back = fft_kernel<IFFTY, io_y_back, id_op, id_op, inter_type>;
    set_kernel_shared_size(kernel_y_back, y_max_bytes);

    auto kernel_z_back = fft_kernel<IFFTZ, io_z_back, id_op, StoreFunctor, inter_type, io_type>;
    ...
    const size_t         flat_fft_size_bytes = flat_fft_size * sizeof(io_type);
    std::vector<io_type> output_host(Batches * flat_fft_size, std::numeric_limits<precision>::quiet_NaN());
    ...
    // Return results
    return example::fft_results<io_type> {output_host, (time / cufftdx_example_performance_runs)};
}
...
int conv_3d() {
    ...
    // The primary sizes of this FFT are not trivially
    // decomposable and must use a slower algorithm.
    // Without a loss in correctness we can pad input
    // with 0s till the closest size not requiring workspace
    ...
    // In case of 51, 64 (power of 2) is the closest "fast" size
    ...
    using fftz_partial = decltype(Block() + Size<fft_size_z_padded>() +
    ...
    auto host_input = example::get_random_real_data<precision>(batches * flat_fft_size, -1, 1);

    // Allocate managed memory for device input/output
    precision* input;
    precision* output;
    const auto flat_fft_size_bytes = flat_fft_size * sizeof(precision);
    ...
    // Run cuFFTDx
    auto cufftdx_results = cufftdx_3d_convolution_padded<batches, fftx_partial, fft_size_x, ffty_partial, fft_size_y, fftz_partial, fft_size_z, load_functor, dx_filter_functor, store_functor>(input, output, stream);
