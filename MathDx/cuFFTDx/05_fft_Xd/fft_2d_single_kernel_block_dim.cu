...
#include "./block_io_generic_strided_block_dim.hpp"
...
// Example showing how cuFFTDx can be used to perform a 2D FFT in a single kernel using a cooperative grid kernel launch.
//
// Notes:
...
void fft_2d() {
    ...
    // FFT Sizes
    // We can use irregular FFT sizes because we will set the block dim to the bigger one
    static constexpr unsigned int fft_size_y = 80;
    static constexpr unsigned int fft_size_x = 100;
    // Kernel Settings
    static constexpr unsigned int ept_y = 10;
    static constexpr unsigned int fpb_y = 10;
    static constexpr unsigned int ept_x = 10;
    static constexpr unsigned int fpb_x = fpb_y; // fpb for X and Y dimensions must be the same due to how IO is implemented


    using namespace cufftdx;
    using fft_base = decltype(Block() + Type<fft_type::c2c>() + Direction<fft_direction::forward>() +
                              Precision<precision_type>() + SM<Arch>());
    using fft_y_base    = decltype(fft_base() + Size<fft_size_y>() + ElementsPerThread<ept_y>() + FFTsPerBlock<fpb_y>());
    using fft_x_base    = decltype(fft_base() + Size<fft_size_x>() + ElementsPerThread<ept_x>() + FFTsPerBlock<fpb_x>());
    // Compare block dims and get the maximum
    constexpr unsigned int block_dim_x = std::max(fft_y_base::block_dim.x, fft_x_base::block_dim.x);
    // Thanks to the block dim operator we can fuse FFTs of different block dim sizes.
    using fft_y      = decltype(fft_y_base() + BlockDim<block_dim_x, fft_y_base::block_dim.y,1>()); // 1 is the number of blocks in the y dimension
    using fft_x      = decltype(fft_x_base() + BlockDim<block_dim_x, fft_x_base::block_dim.y,1>());
    // Print block dims
    std::cout << "Block dims: " << block_dim_x << "\n";
    ...
    if (success) {
        ...
        std::cout << "\nFailure\n";
        for (size_t i = 0; i < flat_fft_size; i++) {
            std::cout << i << ": " << cufftdx_results.output[i].x << " " << cufftdx_results.output[i].y << " " << cufft_results.output[i].x << " " << cufft_results.output[i].y << "\n";
        }
        std::exit(1);
    }
}
