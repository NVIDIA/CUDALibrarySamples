...
# The main intention of this CMake is to provide the client for a way of building the lto helper
# and link the ltoirs with his/her executable.
# Trigger the configuration and build explicitly during the configuration phase
function(build_cufft_lto_helper SRC_DIR BUILD_DIR)
    message(STATUS "Configuring and building 'lto_helper' immediately...")
    set(LTO_COMMAND_ARGS
        -S ${SRC_DIR}
        -B ${BUILD_DIR}
    ...
    # Set Toolchain file or CXX compiler for lto_helper project
    if(DEFINED CUFFTDX_LTO_TOOLCHAIN_FILE)
        list(APPEND LTO_COMMAND_ARGS -DCMAKE_TOOLCHAIN_FILE=${CUFFTDX_LTO_TOOLCHAIN_FILE})
    else()
        if(NOT CMAKE_CROSSCOMPILING)
            if(DEFINED CMAKE_TOOLCHAIN_FILE AND NOT CMAKE_TOOLCHAIN_FILE STREQUAL "")
                list(APPEND LTO_COMMAND_ARGS -DCMAKE_TOOLCHAIN_FILE=${CMAKE_TOOLCHAIN_FILE})
            else()
                list(APPEND LTO_COMMAND_ARGS -DCMAKE_CXX_COMPILER=${CMAKE_CXX_COMPILER})
            endif()
        endif()
    endif()
    ...
    if (DEFINED cufft_ROOT)
        list(APPEND LTO_COMMAND_ARGS -Dcufft_ROOT=${cufft_ROOT})
    elseif(DEFINED cufftdx_cufft_HOME)
        list(APPEND LTO_COMMAND_ARGS -Dcufftdx_cufft_HOME=${cufftdx_cufft_HOME})
    endif()

    # Execute the command
    execute_process(
        COMMAND ${CMAKE_COMMAND} ${LTO_COMMAND_ARGS}
        RESULT_VARIABLE CONFIG_RESULT
    )
    if(NOT CONFIG_RESULT EQUAL 0)
        message(FATAL_ERROR "Configuration of 'lto_helper' failed!")
    endif()
    ...
    if(NOT BUILD_RESULT EQUAL 0)
        message(FATAL_ERROR "Build of 'lto_helper' failed!")
    endif()
    message(STATUS "'lto_helper' has been built successfully.")

    # Register the files in lto_helper as part of the standard clean
    set_property(DIRECTORY APPEND PROPERTY ADDITIONAL_MAKE_CLEAN_FILES "${BUILD_DIR}")
endfunction()

function(run_cufft_lto_helper SRC_DIR BUILD_DIR OUTPUT_NAME DESCS CUDA_ARCHITECTURES FORCE_REGENEARTE)
    ...
    set(CUFFT_LTO_HELPER_RESULT TRUE)  # Assume success initially

    # Check if the artifacts are already generated
    if(NOT EXISTS ${ARTIFACTS_DIR}/lto_database.hpp.inc OR FORCE_REGENEARTE)
        # Remove files under artifacts directory
        file(GLOB FILES_TO_REMOVE "${ARTIFACTS_DIR}/*")
        file(REMOVE ${FILES_TO_REMOVE})

        # Check that executable exists
        if(NOT EXISTS ${BUILD_DIR}/cufftdx_cufft_lto_helper)
            build_cufft_lto_helper(${SRC_DIR} ${BUILD_DIR})
        endif()
        ...
        if(result)
            message(FATAL_ERROR "Execution of cufftdx_cufft_lto_helper failed")
        endif()
    endif()
    ...
    if(NOT LTO_FILES)
        ...
        set(CUFFT_LTO_HELPER_RESULT FALSE)
    else()
    ...
    set(CUFFT_LTO_HELPER_RESULT ${CUFFT_LTO_HELPER_RESULT} PARENT_SCOPE)
endfunction()
