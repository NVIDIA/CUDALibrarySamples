...
void simple_block_fft_c2r_lto() {
    ...
    // - Block() operator informs that FFT will be executed on block level. Shared memory is required for co-operation between threads.
    // - CodeType() operator informs that FFT will attempt to use cuFFT-dumped LTOIRs for computes.
    using FFT_without_code_type = decltype(Block() + Size<128>() + Type<fft_type::c2r>() +
...
template<unsigned int Arch>
struct simple_block_fft_c2r_lto_functor {
    void operator()() { return simple_block_fft_c2r_lto<Arch>(); }
};

int main(int, char**) {
    return example::sm_runner<simple_block_fft_c2r_lto_functor>();
}
