...
// This example demonstrates how to use cuFFTDx to operform a convolution with 1-dimensional complex-to-complex FFTs.
//
// Single CUDA block is run. It performs convolution by doing forward 64-point FFT, pointwise operation,
// and inverse 64-point FFT.
...
void convolution() {
    ...
    // FFT_base defined as a common description of FFT and IFFT.
    // FFT_base is not a complete FFT description. In order to complete FFT description directions are specified:
    // forward for FFT, inverse for IFFT.
    using FFT_base     = decltype(Block() + Size<fft_size>() + Type<fft_type::c2c>() + Precision<float>() + SM<Arch>());
    ...
    complex_type* data;
    auto          size       = FFT::ffts_per_block * fft_size;
    auto          size_bytes = size * sizeof(complex_type);
    CUDA_CHECK_AND_EXIT(cudaMallocManaged(&data, size_bytes));
    for (size_t i = 0; i < size; i++) {
        data[i] = complex_type {float(i+1), -float(i+1)};
    }
