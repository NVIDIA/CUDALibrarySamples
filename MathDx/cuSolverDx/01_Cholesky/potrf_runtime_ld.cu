...
int potrf_runtime_ld() {
    ...
    using Solver = decltype(Size<32, 32>() + Precision<double>() + Type<type::real>() + Function<potrf>() + FillMode<lower>() + Block() + SM<Arch>() + BlockDim<256>()); // column major and lower fill is the default
