...
__global__ __launch_bounds__(Solver::max_threads_per_block) void kernel(DataType* A, unsigned int lda, typename Solver::status_type* info) {
    ...
    common::io<Solver>::load_a(A, lda, As, lda);

    Solver().execute(As, lda, info);

    // Store results back to global memory
    common::io<Solver>::store_a(As, lda, A, lda);
}
...
int getrf_wo_pivot() {
    ...
    common::reference_cusolver_lu<data_type, cuda_data_type, false /*do_solver*/>(A, dummy_B, &info, m, n);
