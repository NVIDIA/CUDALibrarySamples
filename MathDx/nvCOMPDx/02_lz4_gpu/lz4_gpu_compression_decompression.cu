...
__global__ void comp_warp_kernel(
  ...
  constexpr auto tmp_size_warp = compressor.tmp_size_group();
...
static int lz4_gpu_comp_gpu_decomp(
    ...
    if constexpr (DT == datatype::uint16) {
      if (part.size() % 2 != 0) {
        throw std::runtime_error(
          "ERROR: with the selected data type (uint16), all input file sizes must be divisible by 2");
      }
    } else if constexpr (DT == datatype::uint32) {
      if (part.size() % 4 != 0) {
        throw std::runtime_error(
          "ERROR: with the selected data type (uint32), all input file sizes must be divisible by 4");
      }
    }
  }
  ...
  auto comp_temp_bytes = lz4_compressor_type().tmp_size_total(batch_size);
  ...
  auto max_out_bytes = lz4_compressor_type().max_comp_chunk_size();
    ...
    while (i < argc) {
      ...
      if (strcmp(current_argv, "-f") == 0) {
        ...
        if (strcmp(data_type, "uint8") == 0) {
          ...
          std::cerr << "Unknown data type selected (" << data_type
                    << "). Select from the supported options: uint8, uint16, or uint32." << std::endl;
          print_usage();
          return 1;
        }
      } else {
