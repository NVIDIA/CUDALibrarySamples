...
int simple_gemm() {
    ...
    using BLAS = decltype(cublasdx::Size<m, n, k>() + cublasdx::Precision<__half>() +
                          cublasdx::Type<cublasdx::type::complex>() + cublasdx::Function<cublasdx::function::MM>() +
                          cublasdx::Arrangement<cublasdx::row_major, cublasdx::col_major>() + cublasdx::MaxAlignment() +
                          cublasdx::Block() + cublasdx::SM<Arch>());
