...
struct negate {
    template<class T>
    __device__ __host__ constexpr T operator()(const T arg) const {
        return -arg;
    }
};
...
__launch_bounds__(BLAS::max_threads_per_block) //
    ...
    cublasdx::transform_fragment(d_frag, c_load_op);
    cublasdx::axpby(alpha, result_fragment, beta, d_frag);
    cublasdx::transform_fragment(d_frag, c_store_op);
...
int simple_gemm() {
    ...
    using BLAS =
        decltype(cublasdx::Size<m, n, k>() + cublasdx::Precision<__half>() + cublasdx::Type<cublasdx::type::real>() +
                 ...
                 cublasdx::LeadingDimension<lda, ldb, ldc>() + cublasdx::Function<cublasdx::function::MM>() +
    ...
    gemm_kernel<BLAS><<<1, BLAS::block_dim, shared_size>>>(
        a.data(), b.data(), c.data(), alpha, beta, output.data(), a_load_op, b_load_op, c_load_op, c_store_op);
    ...
    // Calculate reference
    cudaStream_t str                   = 0;
    auto         reference_host_output = example::reference_gemm<BLAS>(
        alpha, host_a, host_b, beta, host_c, str, a_load_op, b_load_op, c_load_op, c_store_op);
