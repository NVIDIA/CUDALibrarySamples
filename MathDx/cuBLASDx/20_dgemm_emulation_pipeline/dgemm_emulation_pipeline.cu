...
namespace {
    template<class BLAS, unsigned SwizzleM, class CTensor, class Pipeline>
    __launch_bounds__(Pipeline::max_threads_per_block, 1) __global__
    void pipeline_kernel(double const alpha, double const beta, CTensor global_c,
        ...
        // Get appropriate block tiles from multiblock inputs
    ...
    template<class BLAS, unsigned SwizzleM, class ATensor, class BTensor, class CTensor>
    auto measure_cublasdx_emulated(double const alpha,
                                   double const beta,
        ...
        auto run_cublasdx_gemm = [&](cudaStream_t const str) {
            ...
            auto kernel = pipeline_kernel<BLAS, SwizzleM, CTensor, decltype(pipeline->get_device_handle())>;
            CUDA_CHECK_AND_EXIT(
        ...
        CUDA_CHECK_AND_EXIT(cudaMemcpy(results.data(), c_ptr, results.size() * sizeof(c_value_type), cudaMemcpyDeviceToHost));
        CUDA_CHECK_AND_EXIT(cudaDeviceSynchronize());
...
int dgemm_emulation_pipeline(GlobalShape const global_shape) {
    ...
    constexpr unsigned required_mantissa_bits = 52;
    ...
    using emulated_blas = decltype(cublasdx::Size<tile_m, tile_n, tile_k>() +
                                   cublasdx::Precision<double, double, double>() +
    ...
    auto host_a = example::get_random_data<double>(std::size_t(m) * k, 1);
    auto host_b = example::get_random_data<double>(std::size_t(k) * n, 2);
    auto host_c = example::get_random_data<double>(std::size_t(m) * n, 3);

    example::device_vector<double> device_a = host_a;
    example::device_vector<double> device_b = host_b;
    example::device_vector<double> device_emulated_c = host_c;
    example::device_vector<double> device_cublas_c = host_c;
    ...
    auto [time_cublas, host_blas_results] =
        example::cublaslt_runner<double, double, double>(gemm_shape, global_arrangement, global_ld)
    ...
    const double flops = example::gemm_flops<double, double, double>(m, n, k);
    ...
    std::cout << "Compute Type A: " << example::type_string<double>() << std::endl;
    std::cout << "Compute Type B: " << example::type_string<double>() << std::endl;
    std::cout << "Compute Type C: " << example::type_string<double>() << std::endl;
    std::cout << "Dx Input Precision A: " << example::precision_string<double>() << std::endl;
    std::cout << "Dx Input Precision B: " << example::precision_string<double>() << std::endl;
    std::cout << "Dx Input Precision C: " << example::precision_string<double>() << std::endl;
    ...
    std::cout << "\ncuBLASLt native FP64 (not including heuristic)\n";
    ...
    const bool is_correct = example::is_error_acceptable<double, double, double>(error);
...
struct dgemm_emulation_pipeline_functor {
    ...
    int operator()(std::integral_constant<int, Arch>,
        ...
        return dgemm_emulation_pipeline<Arch, Modifier>(global_shape);
    }
};

int main(int const argc, char** argv) {
    std::array<unsigned int, 3> mnk = {8192, 8192, 8192};
    auto usage = []() { std::cerr << "Incorrect usage: ./dgemm_emulation_pipeline [m n k]" << std::endl; };
    ...
    return example::sm_runner(dgemm_emulation_pipeline_functor {}, cute::make_shape(mnk[0], mnk[1], mnk[2]));
}
