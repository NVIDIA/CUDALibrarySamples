...
namespace detail {
    ...
    template<class T>
    void trsm_transpose_batch(std::vector<T>& data,
                               unsigned        dim_fast,
                               unsigned        dim_slow,
                               unsigned        batches) {
...
auto reference_trsm(const std::vector<T>& h_A,
    ...
    CUDA_CHECK_AND_EXIT(cudaMemcpy(d_A_ptrs, h_A_ptrs.data(),
                                   sizeof(T*) * batches, cudaMemcpyHostToDevice));
    CUDA_CHECK_AND_EXIT(cudaMemcpy(d_B_ptrs, h_B_ptrs.data(),
                                   sizeof(T*) * batches, cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK_AND_EXIT(cudaMemcpyAsync(d_A, a_data,
                                        sizeof(T) * a_sz * batches, cudaMemcpyHostToDevice, stream));
    CUDA_CHECK_AND_EXIT(cudaMemcpyAsync(d_B, b_work.data(),
                                        sizeof(T) * b_sz * batches, cudaMemcpyHostToDevice, stream));

    constexpr bool is_float   = std::is_same_v<T, float> || std::is_same_v<T, cublasdx::complex<float>>;
    constexpr bool is_complex = std::is_same_v<T, cublasdx::complex<float>> ||
                                std::is_same_v<T, cublasdx::complex<double>>;

    auto call_trsm = [&]() {
        if constexpr (is_float && !is_complex) {
            const float a = *reinterpret_cast<const float*>(&alpha);
            CUBLAS_CHECK_AND_EXIT(cublasStrsmBatched(handle, side, uplo, trans, diag,
                                                     static_cast<int>(m), static_cast<int>(n), &a,
                                                     (const float* const*)d_A_ptrs, lda,
                                                     (float* const*)d_B_ptrs, ldb,
                                                     static_cast<int>(batches)));
        } else if constexpr (is_float && is_complex) {
            CUBLAS_CHECK_AND_EXIT(cublasCtrsmBatched(handle, side, uplo, trans, diag,
                                                     static_cast<int>(m), static_cast<int>(n),
                                                     reinterpret_cast<const cuComplex*>(&alpha),
                                                     (const cuComplex* const*)d_A_ptrs, lda,
                                                     (cuComplex* const*)d_B_ptrs, ldb,
                                                     static_cast<int>(batches)));
        } else if constexpr (!is_float && !is_complex) {
            const double a = *reinterpret_cast<const double*>(&alpha);
            CUBLAS_CHECK_AND_EXIT(cublasDtrsmBatched(handle, side, uplo, trans, diag,
                                                     static_cast<int>(m), static_cast<int>(n), &a,
                                                     (const double* const*)d_A_ptrs, lda,
                                                     (double* const*)d_B_ptrs, ldb,
                                                     static_cast<int>(batches)));
        } else {
            CUBLAS_CHECK_AND_EXIT(cublasZtrsmBatched(handle, side, uplo, trans, diag,
                                                     static_cast<int>(m), static_cast<int>(n),
                                                     reinterpret_cast<const cuDoubleComplex*>(&alpha),
                                                     (const cuDoubleComplex* const*)d_A_ptrs, lda,
                                                     (cuDoubleComplex* const*)d_B_ptrs, ldb,
                                                     static_cast<int>(batches)));
        }
    };
    ...
    CUDA_CHECK_AND_EXIT(cudaMemcpyAsync(b_work.data(), d_B,
                                        sizeof(T) * b_sz * batches, cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK_AND_EXIT(cudaStreamSynchronize(stream));
    ...
    if (runs > 0) {
        ...
        CUDA_CHECK_AND_EXIT(cudaMemcpyAsync(d_B, b_orig.data(),
                                            sizeof(T) * b_sz * batches, cudaMemcpyHostToDevice, stream));
        CUDA_CHECK_AND_EXIT(cudaStreamSynchronize(stream));
...
auto reference_trsm(const std::vector<typename BLAS::a_value_type>& h_A,
    ...
    return reference_trsm<T>(h_A, h_B, M, N, batches, is_left, is_lower, is_unit, is_col_a, is_col_b,
                              CUBLAS_OP_N, warm_up_runs, runs, stream);
}
