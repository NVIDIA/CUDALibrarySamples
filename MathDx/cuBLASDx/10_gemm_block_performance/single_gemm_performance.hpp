...
__launch_bounds__(GEMM::max_threads_per_block) __global__ void gemm_kernel(const AInputType*                 a,
    ...
    // Download d data
    auto d_frag_compute = accumulator.make_empty_fragment();
    auto d_frag_io      = accumulator.make_partition_and_copy(c_global_tensor);
    cublasdx::transform_fragment(d_frag_io, d_frag_compute, example::converter<typename GEMM::c_value_type> {});

#pragma unroll 1
    for (unsigned int i = 0; i < repeats; i++) {
        GEMM().execute(a_shared_tensor,
                       b_shared_tensor,
                       accumulator,
        ...
        cublasdx::axpby(alpha, result_fragment, beta, d_frag_compute);
    }

    cublasdx::transform_fragment(d_frag_compute, d_frag_io, example::converter<CInputType> {});
...
int benchmark_mixed_precision_gemm(const cudaStream_t& stream, bool verbose = false) {
    ...
    // Correctness run
    gemm_kernel<gemm_type, TA, TB, TC>
        <<<1, gemm_type::block_dim, shared_memory_size, stream>>>(a, b, c, alpha, beta, output, 1);
    ...
    double ms_n1 = example::measure::execution(
        [&](cudaStream_t stream) {
            gemm_kernel<gemm_type, TA, TB, TC>
                <<<1, gemm_type::block_dim, shared_memory_size, stream>>>(a, b, c, alpha, beta, output, inside_repeats);
        },
    ...
    if (verbose) {
        ...
        std::cout << "Block size: " << gemm_type::block_dim.x << ", " << gemm_type::block_dim.y << ", "
                  << gemm_type::block_dim.z << std::endl;
        ...
        std::cout << "(" << size_of<gemm_type>::m << ", " << size_of<gemm_type>::n << ", " << size_of<gemm_type>::k
                  ...
                  << " B precision " << example::precision_string<TC>() << " c precision " << ": " << std::fixed
                  << std::setprecision(4) << gflops << " GFLOPS, " << avg_time_n << " ms." << std::endl;
    }
