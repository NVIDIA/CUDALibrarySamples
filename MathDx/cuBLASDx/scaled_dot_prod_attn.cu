...
int scaled_dot_product_attention_performance() {
    ...
    value_type* inputs;
    value_type* output;
    auto inputs_size       = BLAS1::a_size + BLAS1::b_size + BLAS1::c_size + BLAS2::b_size;
    ...
    value_type* key       = query + (BLAS1::a_size); // B matrix for BLAS1
    value_type* mask      = key   + (BLAS1::b_size); // C matrix for BLAS1
    value_type* value     = mask  + (BLAS1::c_size); // B matrix for BLAS2

    // Fill the query, key, and value matrices with random values.
    auto host_query = example::get_random_data<value_type>(0.1, 0.5, BLAS1::a_size);
    auto host_key   = example::get_random_data<value_type>(0.1, 0.5, BLAS1::b_size);
    auto host_value = example::get_random_data<value_type>(0.1, 0.5, BLAS2::b_size);
    CUDA_CHECK_AND_EXIT(cudaMemcpy(query, host_query.data(), BLAS1::a_size * sizeof(value_type), cudaMemcpyHostToDevice));
    CUDA_CHECK_AND_EXIT(cudaMemcpy(key, host_key.data(), BLAS1::b_size * sizeof(value_type), cudaMemcpyHostToDevice));
    CUDA_CHECK_AND_EXIT(cudaMemcpy(value, host_value.data(), BLAS2::b_size * sizeof(value_type), cudaMemcpyHostToDevice));
    // Set the mask to 0. (no mask).
    CUDA_CHECK_AND_EXIT(cudaMemset(mask, 0, BLAS1::c_size * sizeof(value_type)));
    CUDA_CHECK_AND_EXIT(cudaDeviceSynchronize());
