...
inline void printPerfStructure(const customMatmulPerf_t &perf) {
    ...
    cublasLtMatmulAlgoConfigGetAttribute(matmulAlgo, CUBLASLT_ALGO_CONFIG_CTA_SWIZZLING, &swizzle, sizeof(swizzle),
                                         NULL);
    ...
    printf("algo={ Id=%d, tileIdx=%d (%s) splitK=%d reduc=%d swizzle=%d custom=%d stages=%d} status %d "
           ...
           algoId, tile, tileToString(tile), numSplitsK, reductionScheme, swizzle, customOption, stages, perf.status,
           perf.time, (int)perf.workspaceSize, (int)perf.mathMode, perf.wavesCount);
}
        ...
        if (heurResult.workspaceSize <= workSpaceSizeInBytes) {
            ...
            for (int loop = 0; loop < repeats; loop++) {
                cublasStatus_t oneRunStatus =
                    cublasLtMatmul(ltHandle, operationDesc, alpha, A, Adesc, B, Bdesc, beta, C, Cdesc, D, Ddesc, &algo,
...
#define ALGO_IDS 16
    ...
    checkCublasStatus(cublasLtMatmulAlgoGetIds(ltHandle, computeType, scaleType, Atype, Btype, Ctype, Dtype, ALGO_IDS,
                                               algoIdA, &nbAlgoIds));
    ...
    for (int idx = 0; (idx < nbAlgoIds) && (AlgoCount < AlgoCombinations); idx++) {
        ...
        if (nbStages == 0) {
            ...
            checkCublasStatus(cublasLtMatmulAlgoCapGetAttribute(&algo, CUBLASLT_ALGO_CAP_STAGES_IDS, stagesA.data(),
        ...
        cublasLtMatmulAlgoCapGetAttribute(&algo, CUBLASLT_ALGO_CAP_TILE_IDS, tileA, sizeof(int) * nbTiles,
                                          &sizeWritten);
        cublasLtMatmulAlgoCapGetAttribute(&algo, CUBLASLT_ALGO_CAP_SPLITK_SUPPORT, &splitkSupport,
                                          sizeof(splitkSupport), &sizeWritten);
        cublasLtMatmulAlgoCapGetAttribute(&algo, CUBLASLT_ALGO_CAP_REDUCTION_SCHEME_MASK, &redMask, sizeof(redMask),
                                          &sizeWritten);
        cublasLtMatmulAlgoCapGetAttribute(&algo, CUBLASLT_ALGO_CAP_CTA_SWIZZLING_SUPPORT, &swizzlingMax,
                                          sizeof(swizzlingMax), &sizeWritten);
        cublasLtMatmulAlgoCapGetAttribute(&algo, CUBLASLT_ALGO_CAP_CUSTOM_OPTION_MAX, &customOptionMax,
                                          sizeof(customOptionMax), &sizeWritten);

        // Loop over the different tiles
        for (int tileIdx = 0; tileIdx < nbTiles; tileIdx++) {
            checkCublasStatus(cublasLtMatmulAlgoConfigSetAttribute(&algo, CUBLASLT_ALGO_CONFIG_TILE_ID, &tileA[tileIdx],
            ...
            for (int stagesIdx = 0; stagesIdx < nbStages; stagesIdx++) {
                ...
                for (uint16_t cgaIdx = 0; cgaIdx < cgaLastShape; cgaIdx++) {
                    checkCublasStatus(cublasLtMatmulAlgoConfigSetAttribute(&algo, CUBLASLT_ALGO_CONFIG_CLUSTER_SHAPE_ID,
                                                                           &cgaIdx, sizeof(cgaIdx)));
                    ...
                    for (int customOption = 0; customOption <= customOptionMax; customOption++) {
                        ...
                        for (int k = 0; k <= swizzlingMax; k++) {
                            ...
                            // Loop over the splitK value over a fixed sequence splitKSequenceA in addtion to the case
                            ...
                            for (int l = 0; (l < (1 + splitK_trial)) && (AlgoCount < AlgoCombinations); l++) {
                                ...
                                if (l > 0) { // Split-K case
                                    ...
                                    checkCublasStatus(cublasLtMatmulAlgoConfigSetAttribute(
                                        &algo, CUBLASLT_ALGO_CONFIG_SPLITK_NUM, &splitKSequenceA[l - 1],
                                        sizeof(splitKSequenceA[l - 1])));
                                    // Going over all the reduction schemes
                                    for (redScheme = 1; redScheme < (int)CUBLASLT_REDUCTION_SCHEME_MASK &&
                                        ...
                                        if (redScheme & redMask) {
                                            ...
                                            status = customMatmulRun(ltHandle, operationDesc,
                                                                     alpha, // host or device pointer
                                                                     A, Adesc, B, Bdesc,
                                                                     beta, // host or device pointer
                                                                     C, Cdesc, D, Ddesc, algo, kernelRepeats, workSpace,
