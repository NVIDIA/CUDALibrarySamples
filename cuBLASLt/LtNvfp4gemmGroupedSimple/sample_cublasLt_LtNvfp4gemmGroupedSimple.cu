...
///
/// Sample wrapper executing grouped NVFP4 gemm with cublasLtMatmul, using VEC16_UE4M3 block scaling.
...
void LtNvfp4gemmGroupedSimple(cublasLtHandle_t ltHandle,
                              ...
                              int64_t avgK,
                              const float *alpha,
                              const __nv_fp8_e4m3 *const *a_scale,
                              const typename StorageType<__nv_fp4_e2m1>::type *const *A,
                              const void *ldaArrayDev,
                              const __nv_fp8_e4m3 *const *b_scale,
                              const typename StorageType<__nv_fp4_e2m1>::type *const *B,
    ...
    // set block scaling mode (VEC16_UE4M3 for NVFP4)
    ...
    // set scaling factors (A, B only - D is bf16, no scaling needed)
    checkCublasStatus(
    ...
    checkCublasStatus(cublasLtGroupedMatrixLayoutCreate(&Adesc, CUDA_R_4F_E2M1, batchCount,
                                                        transa == CUBLAS_OP_N ? mArrayDev : kArrayDev,
                                                        transa == CUBLAS_OP_N ? kArrayDev : mArrayDev, ldaArrayDev));
    checkCublasStatus(cublasLtGroupedMatrixLayoutCreate(&Bdesc, CUDA_R_4F_E2M1, batchCount,
