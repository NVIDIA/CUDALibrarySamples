 ...
 *  - CUBLAS_EMULATE_DOUBLE_PRECISION: A value of 1 will allow cublasLt to utilize FP64 emulation algorithms in double
 * precision and double complex precision routines.
 *
 ...
 *                                                    if an emulation algorithm is used, it will use fixed emulation
 * instead of emulation with the number of mantissa bits specified by the user. If an invalid value is set, the
 * user-provided value will be ignored.
 */
int main() {
    /*
     ...
     *
     * See getFixedPointWorkspaceSizeInBytes() in
     * https://docs.nvidia.com/cuda/cublas/index.html#fixed-point-workspace-requirements for more details on how much
    ...
    props.run([&props, &emulationStrategy, &specialValuesSupport, &mantissaControl, &maxMantissaBitCount,
               &mantissaBitOffset] {
        LtDgemmEmulated(props.ltHandle, CUBLAS_OP_N, CUBLAS_OP_N, props.m, props.n, props.k, &props.alpha, props.Adev,
                        props.m, props.Bdev, props.k, &props.beta, props.Cdev, props.m, props.workspace,
                        props.workspaceSize, emulationStrategy, specialValuesSupport, mantissaControl,
                        maxMantissaBitCount, mantissaBitOffset, props.stream);
    });
