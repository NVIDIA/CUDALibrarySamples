...
template <typename T>  
void trtri(cusolverDnHandle_t handle, cusolver_int_t n, T* d_A, cusolver_int_t lda, cublasFillMode_t uplo, cublasDiagType_t diag, int* d_info)
{
    void* d_work = nullptr;
    size_t d_lwork = 0;
    void* h_work = nullptr;
    ...
    try
    {
        ...
        CUDA_CHECK(cudaMalloc((void**)&d_work, d_lwork));
...
int main(int argc, char* argv[])
{
    ...
    CUDA_CHECK(cudaMalloc((void**)&d_A, sizeof(data_type) * lda * n));
    CUDA_CHECK(cudaMalloc((void**)&d_A_inv, sizeof(data_type) * lda * n));
