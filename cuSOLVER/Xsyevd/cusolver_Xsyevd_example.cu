...
int main(int argc, char *argv[]) {
     ...
     *
     */
    const std::vector<data_type> A = {3.5, 0.5, 0, 0.5, 3.5, 0, 0, 0, 2.0};
    const std::vector<data_type> lambda = {2.0, 3.0, 4.0};

    std::vector<data_type> V(lda * m); // eigenvectors
    std::vector<data_type> W(m);       // eigenvalues
    ...
    CUDA_CHECK(cudaMalloc(reinterpret_cast<void **>(&d_W), sizeof(data_type) * m));
    ...
    CUSOLVER_CHECK(cusolverDnXsyevd(
        ...
        h_work, h_lwork, devInfo));

    CUDA_CHECK(cudaMemcpyAsync(V.data(), d_A, sizeof(data_type) * lda * m, cudaMemcpyDeviceToHost,
                               stream));
    CUDA_CHECK(
        cudaMemcpyAsync(W.data(), d_W, sizeof(data_type) * m, cudaMemcpyDeviceToHost, stream));
