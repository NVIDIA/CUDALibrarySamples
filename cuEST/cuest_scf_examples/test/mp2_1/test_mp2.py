...
from mp2_kernel_helper import MP2KernelHelper
...
molecule = cuest_scf.Molecule.parse_from_xyz_file(os.path.join(filedir, 'paxlovid.xyz'))
charge = 0
...
def test_mp2_energy():
    ...
    assert abs(rhf.compute_energy() - -1758.3044466759715760) < 1.0e-6

    print(f"Integral Direct Results")
    ...
    assert abs(mp2_os_energy - -3.9391198091455015) < 1.0e-6
    assert abs(mp2_ss_energy - -1.4027673840159569 ) < 1.0e-6

    print(f"Cached Integral Results")
    mp2_os_energy, mp2_ss_energy = compute_mp2_energy(
        rhf=rhf,
        ri_auxiliary=ri_auxiliary,
        ri_auxiliary_name=ri_auxiliary_name,
