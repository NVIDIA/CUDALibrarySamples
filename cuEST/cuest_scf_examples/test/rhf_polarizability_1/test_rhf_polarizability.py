...
from cphf_kernel_helper import CPHFKernelHelper
...
# Test of polarizability for provided ethanol geometry, HF/def2-SVP, HF/def2-TZVP, HF/def2-QZVP
alpha_refs = {
    'def2-svp':  np.array([[25.25882087983122304,  0.04558107114108978,  0.28241688150043409],
                        [ 0.04558107114995913, 26.64583590928795331, -1.26090587670713794],
                        [ 0.28241688150339606, -1.26090587668958243, 24.69206909672833206]]),
    'def2-tzvp': np.array([[29.13131031608632426,  0.47353599291738346,  0.22749318818442998],
                        [ 0.47353599288996406, 30.06216721876641174, -0.89016172927663129],
                        [ 0.22749318817120923, -0.89016172926982229, 27.63520292891469410]]),
    'def2-qzvp': np.array([[31.0983272222269420,  0.7057206787902999,  0.1415183594176792],
                        [ 0.7057206787044024, 31.5293218732110816, -0.6935529232289035],
                        [ 0.1415183594034346, -0.6935529231941260, 28.7228815666626716]]),
    }

def test_rhf_polarizability():

    threshold_pq = 1.0E-18
    
    molecule_filename = os.path.join(filedir, 'ethanol.xyz')
    molecule = cuest_scf.Molecule.parse_from_xyz_file(molecule_filename)
    charge = 0
    
    for primary_name in ['def2-svp', 'def2-tzvp', 'def2-qzvp']:
        ...
        print(alpha)
        print(alpha_refs[primary_name])
        
        dA = np.max(np.abs(alpha - alpha_refs[primary_name]))
