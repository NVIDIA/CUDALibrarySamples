 ...
 * to evaluate Coulomb and exchange matrices. The use of density-fitting
 ...
 *
 * Formation of the Coulomb matrix takes a density matrix as input. The
 * exchange matrix takes the occupied molecular orbital coefficients as
 * input. 
 *
...
int main(int argc, char **argv)
{
    ...
    /* Allocate space for J and K matrices */
    double *d_J, *d_K;
    if (cudaMalloc((void**) &d_J, nao * nao * sizeof(double))) {
        fprintf(stderr, "Failed to allocate device buffer\n");
        exit(EXIT_FAILURE);
    }
    if (cudaMalloc((void**) &d_K, nao * nao * sizeof(double))) {
        fprintf(stderr, "Failed to allocate device buffer\n");
        exit(EXIT_FAILURE);
    }
    ...
    /* Declare the DF Coulomb compute parameter handle. */
    cuestDFCoulombComputeParameters_t dfj_compute_parameters;
    checkCuestErrors(cuestParametersCreate(
        CUEST_DFCOULOMBCOMPUTE_PARAMETERS, 
        &dfj_compute_parameters));

    /* Workspace query to find the temporary space needed to form the J matrix. */
    ...
    cuestWorkspace_t* temporaryDFJWorkspace = allocateWorkspace(temporaryWorkspaceDescriptor);

    /* Compute J Matrix */
    checkCuestErrors(cuestDFCoulombCompute(
        handle, 
        dfint_plan, 
        dfj_compute_parameters,
        temporaryDFJWorkspace, 
        d_D,
        d_J));

    freeWorkspace(temporaryDFJWorkspace);

    /* The DF Coulomb compute parameter handle is no longer needed. */
    checkCuestErrors(cuestParametersDestroy(
        CUEST_DFCOULOMBCOMPUTE_PARAMETERS, 
        dfj_compute_parameters));
    ...
    /* Workspace query to find the temporary space needed to form the K matrix. */
    ...
    /* Free J, K, D, and Cocc matrices */
    if (cudaFree((void*) d_J) != cudaSuccess) {
