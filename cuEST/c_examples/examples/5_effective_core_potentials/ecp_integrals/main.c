 ...
 * ECP integrals.
 * 
 * This introduces the ECP plan handle, which constructs 
 * a grid which is used to compute the ECP potential. The ECP
 * potential itself is generated directly into a user
...
int main(int argc, char **argv)
{
    ...
    /* Allocate space for ECP matrix */
    double *d_ECP;
    if (cudaMalloc((void**) &d_ECP, nao * nao * sizeof(double))) {
        fprintf(stderr, "Failed to allocate device buffer\n");
        exit(EXIT_FAILURE);
    }

    /* Declare the ECP compute parameter handle. */
    cuestECPComputeParameters_t ecpcompute_parameters;

    /* Create the ECP compute parameters. */
    checkCuestErrors(cuestParametersCreate(
        CUEST_ECPCOMPUTE_PARAMETERS, 
    ...
    checkCuestErrors(cuestECPComputeWorkspaceQuery(
        handle,
        ecpint_plan,
        ecpcompute_parameters,
        variableBufferSize,
    ...
    /* Compute ECP Integrals */
    checkCuestErrors(cuestECPCompute(
        handle,
        ecpint_plan,
        ecpcompute_parameters,
        variableBufferSize,
    ...
    double *h_ECP = (double*) malloc(nao * nao * sizeof(double));
    if(cudaMemcpy(h_ECP, d_ECP, nao * nao * sizeof(double), cudaMemcpyDeviceToHost) != cudaSuccess) {
    ...
    /* Free ECP matrix */
    if (cudaFree((void*) d_ECP) != cudaSuccess) {
