...
/*
 * This example shows how to evaluate the derivative of the nonlocal 
 * exchange correlation energy. This depends on an AO basis set, a 
 * molecular grid (here, a 75, 302 grid) and coefficient matrices. In 
...
int main(int argc, char **argv)
{
    ...
    /**************************************************/
    /* Configure the desired RKS nonlocal derivative */
    /**************************************************/

    cuestNonlocalXCDerivativeRKSComputeParameters_t vv10RKSDerivativeParameters;
    checkCuestErrors(cuestParametersCreate(
        CUEST_NONLOCALXCDERIVATIVERKSCOMPUTE_PARAMETERS, 
        &vv10RKSDerivativeParameters));
    ...
    checkCuestErrors(cuestNonlocalXCDerivativeRKSComputeWorkspaceQuery(
        handle,
        xcIntPlan, 
        vv10RKSDerivativeParameters,
        variableBufferSize, 
    ...
    /* Compute Exc and Vxc */
    checkCuestErrors(cuestNonlocalXCDerivativeRKSCompute(
        handle,
        xcIntPlan, 
        vv10RKSDerivativeParameters,
        variableBufferSize, 
    ...
    /* Nonlocal parameters are no longer needed. */
    checkCuestErrors(cuestParametersDestroy(
        CUEST_NONLOCALXCDERIVATIVERKSCOMPUTE_PARAMETERS,
        vv10RKSDerivativeParameters));

    /* Free gradient, D, and Cocc matrices */
    ...
    /**************************************************/
    /* Configure the desired UKS nonlocal derivative */
    /**************************************************/

    cuestNonlocalXCDerivativeUKSComputeParameters_t vv10UKSDerivativeParameters;
    checkCuestErrors(cuestParametersCreate(
        CUEST_NONLOCALXCDERIVATIVEUKSCOMPUTE_PARAMETERS, 
        &vv10UKSDerivativeParameters));
    ...
    /* Workspace query to find the temporary space needed to form Exc and Vxc. */
    checkCuestErrors(cuestNonlocalXCDerivativeUKSComputeWorkspaceQuery(
        handle,
        xcIntPlan, 
        vv10UKSDerivativeParameters,
        variableBufferSize, 
    ...
    /* Compute Exc and Vxc */
    checkCuestErrors(cuestNonlocalXCDerivativeUKSCompute(
        handle,
        xcIntPlan, 
        vv10UKSDerivativeParameters,
        variableBufferSize, 
