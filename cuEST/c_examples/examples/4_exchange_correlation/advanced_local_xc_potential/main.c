...
/*
 * This example shows how to evaluate the exchange correlation potential using
 * the advanced interface. In this approach, the (rate-limiting) density
 * collocation and its adjoint are performed using cuEST on the GPU, while
 * the density and its derivatives are shuttled to the CPU for the
 * (relatively cheap) evaluation of the functional and its partial derivatives.
...
void run_ansatz(cuestHandle_t handle, parsedXYZFile_t* xyzData, AtomShellData_t* shellData, cuestXCAdvancedComputeParametersApproximation_t ansatz)
{
    ...
    // => Compute the XC energy and integrated density <= //
    double Exc = 0.0;
    double integrated_density = 0.0;
    for (int point = 0; point < npoint; point++) {
        Exc += 2 * h_weights[point] * f[point] * rho_0[point];
    ...
    checkCuestErrors(cuestParametersCreate(
        CUEST_XCPOTENTIALCOMPUTE_PARAMETERS,
        &potential_compute_parameters));
    checkCuestErrors(cuestXCPotentialComputeWorkspaceQuery(
        handle,
        xcIntPlan,
        ansatz,
        potential_compute_parameters,
    ...
    temporaryWorkspace = allocateWorkspace(temporaryWorkspaceDescriptor);
    checkCuestErrors(cuestXCPotentialCompute(
        handle,
        xcIntPlan,
        ansatz,
        potential_compute_parameters,
    ...
    /* Allocate space for Vxc matrix */
    double *d_VxcA;
    if (cudaMalloc((void**) &d_VxcA, nao * nao * sizeof(double))) {
        fprintf(stderr, "Failed to allocate device buffer\n");
        exit(EXIT_FAILURE);
    }
    double *d_VxcB;
    if (cudaMalloc((void**) &d_VxcB, nao * nao * sizeof(double))) {
        fprintf(stderr, "Failed to allocate device buffer\n");
        exit(EXIT_FAILURE);
    }
    ...
    for (int point = 0; point < npoint; point++) {
        Exc += h_weights[point] * f[point] * (rho_0_a[point] + rho_0_b[point]);
    ...
    /* Free Vxc and Cocc matrices */
    if (cudaFree((void*) d_VxcA) != cudaSuccess) {
        fprintf(stderr, "cudaFree failed\n");
        exit(EXIT_FAILURE);
    }
    if (cudaFree((void*) d_VxcB) != cudaSuccess) {
        fprintf(stderr, "cudaFree failed\n");
        exit(EXIT_FAILURE);
    }
