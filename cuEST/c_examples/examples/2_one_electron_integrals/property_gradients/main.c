 ...
 * a non-symmetric density matrix.
 */
void fill_nonsymmetric_matrix(double *A, uint64_t N)
{
    ...
    for (uint64_t i=0; i<N; i++) {
        for (uint64_t j=0; j<N; j++) {
            ...
            Atmp[i*N+j] = sqrt(-2.0 * log(u1)) * cos(2.0 * M_PI * u2);
        }
    }
...
int main(int argc, char **argv)
{
    ...
    /* Allocate space for dL/dR, dN/dR, dM/dR, and density matrices */
    double *d_dLdR, *d_dNdR, *d_dMdR, *d_D;
    if (cudaMalloc((void**) &d_dLdR, numAtoms * 3 * sizeof(double))) {
        fprintf(stderr, "Failed to allocate device buffer\n");
        exit(EXIT_FAILURE);
    }
    if (cudaMalloc((void**) &d_dNdR, numAtoms * 3 * sizeof(double))) {
        fprintf(stderr, "Failed to allocate device buffer\n");
        exit(EXIT_FAILURE);
    }
    if (cudaMalloc((void**) &d_dMdR, numAtoms * 3 * sizeof(double))) {
        fprintf(stderr, "Failed to allocate device buffer\n");
        exit(EXIT_FAILURE);
    }
    ...
    fill_nonsymmetric_matrix(d_D, nao);
    ...
    cuestAngularMomentumDerivativeComputeParameters_t angular_momentum_compute_parameters;
    checkCuestErrors(cuestParametersCreate(
        CUEST_ANGULARMOMENTUMDERIVATIVECOMPUTE_PARAMETERS,
        &angular_momentum_compute_parameters));

    /* Workspace query to find the temporary space needed to form the L derivative. */
    checkCuestErrors(cuestAngularMomentumDerivativeComputeWorkspaceQuery(
        handle,
        oeint_plan,
        angular_momentum_compute_parameters,
        temporaryWorkspaceDescriptor,
    ...
    /* Compute L Derivative */
    checkCuestErrors(cuestAngularMomentumDerivativeCompute(
        handle,
        oeint_plan,
        angular_momentum_compute_parameters,
        temporaryLWorkspace,
    ...
    cuestNablaDerivativeComputeParameters_t nabla_compute_parameters;
    checkCuestErrors(cuestParametersCreate(
        CUEST_NABLADERIVATIVECOMPUTE_PARAMETERS,
        &nabla_compute_parameters));

    /* Workspace query to find the temporary space needed to form the N derivative. */
    checkCuestErrors(cuestNablaDerivativeComputeWorkspaceQuery(
        handle,
        oeint_plan,
        nabla_compute_parameters,
        temporaryWorkspaceDescriptor,
    ...
    /* Compute N Derivative */
    checkCuestErrors(cuestNablaDerivativeCompute(
        handle,
        oeint_plan,
        nabla_compute_parameters,
        temporaryNWorkspace,
    ...
    cuestMultipoleDerivativeComputeParameters_t multipole_compute_parameters;
    checkCuestErrors(cuestParametersCreate(
        CUEST_MULTIPOLEDERIVATIVECOMPUTE_PARAMETERS,
        &multipole_compute_parameters));

    /* Workspace query to find the temporary space needed to form the M derivative. */
    checkCuestErrors(cuestMultipoleDerivativeComputeWorkspaceQuery(
        handle,
        oeint_plan,
        multipole_compute_parameters,
        temporaryWorkspaceDescriptor,
    ...
    /* Compute M Derivative */
    checkCuestErrors(cuestMultipoleDerivativeCompute(
        handle,
        oeint_plan,
        multipole_compute_parameters,
        temporaryMWorkspace,
    ...
    /* Free dLdR, dNdR, dMdR, and density matrices */
    if (cudaFree((void*) d_dLdR) != cudaSuccess) {
        fprintf(stderr, "cudaFree failed\n");
        exit(EXIT_FAILURE);
    }
    if (cudaFree((void*) d_dNdR) != cudaSuccess) {
        fprintf(stderr, "cudaFree failed\n");
        exit(EXIT_FAILURE);
    }
    if (cudaFree((void*) d_dMdR) != cudaSuccess) {
        fprintf(stderr, "cudaFree failed\n");
        exit(EXIT_FAILURE);
    }
