...
def run(
    *,
    ...
    # => Compute Nonsymmetric K Matrices <= #

    # Get nocc for a neutral molecule. Zs are negated charges from the parser.
    nocc = int(np.abs(np.sum(Zs) // 2))

    num_coefficient_matrices = 4

    # Fill with random data. In real use cases these are occupied MO coefficients.
    np.random.seed(0)
    cleft_host_array = np.random.normal(size=(nocc, nao)).astype(np.double).ravel()
    cright_host_array = np.random.normal(size=(num_coefficient_matrices * nocc, nao)).astype(np.double).ravel()
    ...
    cright_device_pointer = cuda_malloc(
        size_in_bytes=sizeof_double * num_coefficient_matrices * nocc * nao
        )
    ...
    exchangematrix_device_pointer = cuda_malloc(
        size_in_bytes=sizeof_double * num_coefficient_matrices * nao * nao
        )
    exchangematrix_device_handle = ce.Pointer(exchangematrix_device_pointer)
    ...
    dfk_compute_parameters = ce.cuestDFNonsymmetricExchangeComputeParameters()
    cuest_check('Create Nonsymmetric Exchange Params',
        ce.cuestParametersCreate(
            parametersType=ce.CuestParametersType.CUEST_DFNONSYMMETRICEXCHANGECOMPUTE_PARAMETERS,
            outParameters=dfk_compute_parameters,
            )
        )

    cuest_check('Compute Nonsymmetric Exchange Workspace Query',
        ce.cuestDFNonsymmetricExchangeComputeWorkspaceQuery(
            handle=cuest_handle,
            plan=dfintplan_handle,
            parameters=dfk_compute_parameters,
            variableBufferSize=variable_buffer_size.pointer,
            temporaryWorkspaceDescriptor=temporary_workspace_descriptor.pointer,
            numCoefficientMatrices=num_coefficient_matrices,
            numOccupied=nocc,
            leftCoefficientMatrix=cleft_device_handle,
            rightCoefficientMatrices=cright_device_handle,
            outExchangeMatrices=exchangematrix_device_handle,
            )
        )

    exchange_int_temporary_workspace = Workspace(workspaceDescriptor=temporary_workspace_descriptor)

    cuest_check('Compute Nonsymmetric Exchange',
        ce.cuestDFNonsymmetricExchangeCompute(
            handle=cuest_handle,
            plan=dfintplan_handle,
            parameters=dfk_compute_parameters,
            variableBufferSize=variable_buffer_size.pointer,
            temporaryWorkspace=exchange_int_temporary_workspace.pointer,
            numCoefficientMatrices=num_coefficient_matrices,
            numOccupied=nocc,
            leftCoefficientMatrix=cleft_device_handle,
            rightCoefficientMatrices=cright_device_handle,
    ...
    del exchange_int_temporary_workspace

    cuest_check('Destroy Nonsymmetric Exchange Params',
        ce.cuestParametersDestroy(
            parametersType=ce.CuestParametersType.CUEST_DFNONSYMMETRICEXCHANGECOMPUTE_PARAMETERS,
            parameters=dfk_compute_parameters,
            )
    ...
    exchangematrix_host_array = np.empty(
        (num_coefficient_matrices * nao * nao,),
        dtype=np.double
        )
