...
int main(int argc, char* argv[])
{
    ...
    cusolverStat = cusolverMpCreateMatrixDesc(&descA, grid, CUDA_R_64F, m, n, mbA, nbA, rsrc, csrc, lldA);
    SAMPLE_ASSERT(cusolverStat == CUSOLVER_STATUS_SUCCESS);
    cusolverStat = cusolverMpCreateMatrixDesc(&descU, grid, CUDA_R_64F, m, k, mbA, nbA, rsrc, csrc, lldU);
    SAMPLE_ASSERT(cusolverStat == CUSOLVER_STATUS_SUCCESS);
    cusolverStat = cusolverMpCreateMatrixDesc(&descVT, grid, CUDA_R_64F, k, n, mbA, nbA, rsrc, csrc, lldVT);
    SAMPLE_ASSERT(cusolverStat == CUSOLVER_STATUS_SUCCESS);
    ...
    cusolverStat         = cusolverMpGesvdDescriptorSetAttribute(gesvdDesc,
    ...
    cusolverStat = cusolverMpGesvd_bufferSize(handle,
                                              gesvdDesc,
                                              jobu,
                                              jobvt,
                                              m,
    ...
    MPI_Allreduce(MPI_IN_PLACE, &d_workSize, 1, MPI_UNSIGNED_LONG, MPI_MAX, MPI_COMM_WORLD);
    if (d_workSize > 0)
    {
        ...
        ncclStat = ncclMemAlloc(&d_work, d_workSize);
        ...
        cusolverStat = cusolverMpBufferRegister(grid, d_work, d_workSize);
     ...
     * ================================================================ */
    cusolverStat = cusolverMpGesvd(handle,
                                   gesvdDesc,
                                   jobu,
                                   jobvt,
                                   m,
    ...
    if (rank == 0)
    {
        ...
        if (h_info < 0)
            ...
            msg = "singular-value computation did not complete";
        else if (h_info == 2)
            msg = "singular-vector computation did not complete";
        printf("\nResults:\n");
        printf("  d_info = %d (%s)\n", h_info, msg);
    }
    ...
    if (rank == 0)
    {
        ...
        cusolverStat         = cusolverMpGesvdDescriptorGetAttribute(
                gesvdDesc, CUSOLVERMP_GESVD_DESCRIPTOR_ATTRIBUTE_A_NORM_FROBENIUS, &a_nrmF, sizeof(a_nrmF), &written);
        SAMPLE_ASSERT(cusolverStat == CUSOLVER_STATUS_SUCCESS);
        SAMPLE_ASSERT(written == sizeof(a_nrmF));
        cusolverStat = cusolverMpGesvdDescriptorGetAttribute(
                gesvdDesc, CUSOLVERMP_GESVD_DESCRIPTOR_ATTRIBUTE_RCOND, &rcond, sizeof(rcond), &written);
        SAMPLE_ASSERT(cusolverStat == CUSOLVER_STATUS_SUCCESS);
        SAMPLE_ASSERT(written == sizeof(rcond));
        cusolverStat =
                cusolverMpGesvdDescriptorGetAttribute(gesvdDesc,
        ...
        cusolverStat = cusolverMpGesvdDescriptorGetAttribute(gesvdDesc,
    ...
    cusolverStat = cusolverMpMatrixGatherD2H(handle, m, k, (void*)d_U, 1, 1, descU, 0, (void*)h_U, m);
    SAMPLE_ASSERT(cusolverStat == CUSOLVER_STATUS_SUCCESS);
    cusolverStat = cusolverMpMatrixGatherD2H(handle, k, n, (void*)d_VT, 1, 1, descVT, 0, (void*)h_VT, k);
    SAMPLE_ASSERT(cusolverStat == CUSOLVER_STATUS_SUCCESS);
    ...
    if (d_work)
    {
        ...
        if (workRegistered)
        {
            cusolverStat = cusolverMpBufferDeregister(grid, d_work);
            SAMPLE_ASSERT(cusolverStat == CUSOLVER_STATUS_SUCCESS);
        }
