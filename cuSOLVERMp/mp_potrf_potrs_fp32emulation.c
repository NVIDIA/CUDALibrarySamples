...
int main(int argc, char* argv[])
{
    ...
    cusolverStat = cusolverMpSetMathMode(handle, CUSOLVER_FP32_EMULATED_BF16X9_MATH);
    SAMPLE_ASSERT(cusolverStat == CUSOLVER_STATUS_SUCCESS);

    // explicitly set the emulation strategy to default (redundant)
    cusolverStat = cusolverMpSetEmulationStrategy(handle, CUDA_EMULATION_STRATEGY_DEFAULT);
    SAMPLE_ASSERT(cusolverStat == CUSOLVER_STATUS_SUCCESS);
