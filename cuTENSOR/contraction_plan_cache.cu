...
try
{
    ...
    double gflops = (2.0 * extent['m'] * extent['n'] * extent['u'] * extent['v'] * extent['k'] * extent['h']) /1.e9;
    ...
    auto work = cuda_alloc<char>(0); // Start with a buffer of size 0.
    ...
    for (int i=0; i < incCount + 1; ++i) // last iteration will hit the cache
    {
        ...
        {
            work = nullptr; // Releases old workspace.
            work = cuda_alloc<char>(actualWorkspaceSize); // Allocates new workspace.
        ...
        handle_error(cutensorContract(handle,
                                  ...
                                  work.get(), actualWorkspaceSize, 0 /* stream */));
