...
int main(const int argc, const char** argv) {
    ...
    if (argc != 6){
        printf("Usage: %s --A_num_rows=## --A_num_cols=## --B_num_cols=## --A_sparsity=0.## --num_batches=##\n", argv[0]);
        return EXIT_FAILURE;
    }
    printf("A_num_rows: %d\n", A_num_rows);
    printf("A_num_cols: %d\n", A_num_cols);
    ...
    for (int idx = 0; idx<num_batches; idx++){
        CHECK_CUDA( cudaMemcpy(dA_rows + idx * A_nnz , (void *)thrust::raw_pointer_cast(dA.row_indices.data()), A_nnz * sizeof(int),
                            cudaMemcpyDeviceToDevice) )
        CHECK_CUDA( cudaMemcpy(dA_columns + idx * A_nnz , (void *)thrust::raw_pointer_cast(dA.column_indices.data()), A_nnz * sizeof(int),
                            cudaMemcpyDeviceToDevice) )
        CHECK_CUDA( cudaMemcpy(dA_values + idx * A_nnz , (void *)thrust::raw_pointer_cast(dA.values.data()), A_nnz * sizeof(float),
                            cudaMemcpyDeviceToDevice) )
    ...
    printf("cusparseSpMM batched time (microseconds): %ld\n",
