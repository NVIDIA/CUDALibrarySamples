...
int main(void) {
    ...
    //                        cudaMemcpyHostToDevice) )
    //--------------------------------------------------------------------------
    cusp::csr_matrix<int, float, cusp::host_memory> hC(A_num_rows, B_num_cols, C_nnz);
    std::copy(hC_offsets, hC_offsets + A_num_rows + 1, thrust::raw_pointer_cast(hC.row_offsets.data()));
    std::copy(hC_columns, hC_columns + C_nnz, thrust::raw_pointer_cast(hC.column_indices.data()));
    std::copy(hC_values, hC_values + C_nnz, thrust::raw_pointer_cast(hC.values.data()));
