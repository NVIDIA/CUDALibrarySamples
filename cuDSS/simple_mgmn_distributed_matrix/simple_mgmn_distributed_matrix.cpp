...
int main(int argc, char *argv[]) {
    ...
    if (argc > 2) {
        ...
        if (rank == 0) {
            printf(
                "Error: this example requires passing at least one argument:\n"
    ...
    CUDSS_CALL_AND_CHECK(cudssMatrixSetDistributionRow1d(b, b_row_start, b_row_end),
                         status, "cudssMatrixSetDistributionRow1d");

    CUDSS_CALL_AND_CHECK(cudssMatrixCreateDn(&x, nrows, nrhs, ldx, x_values_d,
                                             CUDSS_R_64F, CUDSS_LAYOUT_COL_MAJOR),
                         status, "cudssMatrixCreateDn for x");
    /* Set row-wise 1D distribution for  X */
    CUDSS_CALL_AND_CHECK(cudssMatrixSetDistributionRow1d(x, x_row_start, x_row_end),
                         status, "cudssMatrixSetDistributionRow1d");
    ...
    CUDSS_CALL_AND_CHECK(
        cudssMatrixSetDistributionRow1d(A, matrix_row_start, matrix_row_end), status,
        "cudssMatrixSetDistributionRow1d");

    /* Setting communicator to be used by MGMN mode of cuDSS */
#ifdef USE_MPI
    CUDSS_CALL_AND_CHECK(cudssDataSet(handle, solverData, CUDSS_DATA_COMM_HOST, mpi_comm,
                                      sizeof(MPI_Comm *)),
                         status, "cudssDataSet for OpenMPI comm (host)");
#if USE_OPENMPI
    if (strcmp(comm_backend_name, "openmpi") == 0) {
        CUDSS_CALL_AND_CHECK(cudssDataSet(handle, solverData, CUDSS_DATA_COMM_DEVICE,
    ...
    /* (optional) For the root process, print the solution and compare against the exact
     * solution */
    if (rank == 0 || rank == 1) {
