...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < batchCount; i++) {
        ...
        CUDA_CALL_AND_CHECK(
            cudaMalloc(&batch_x_values_h[i], nrhs[i] * n[i] * sizeof(double)),
    ...
    CUDA_CALL_AND_CHECK(cudaMemcpy(batch_b_values_d, batch_b_values_h,
                                   batchCount * sizeof(double *), cudaMemcpyHostToDevice),
                        "cudaMemcpy for b_values");
    CUDA_CALL_AND_CHECK(cudaMemcpy(batch_x_values_d, batch_x_values_h,
                                   batchCount * sizeof(double *), cudaMemcpyHostToDevice),
    ...
    CUDSS_CALL_AND_CHECK(cudssMatrixCreateBatchDn(&x, batchCount, ncols, nrhs, ldx,
                                                  batch_x_values_d, CUDSS_R_32I,
                                                  CUDSS_R_64F, CUDSS_LAYOUT_COL_MAJOR),
