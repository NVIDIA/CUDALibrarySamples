...
int main(int argc, char *argv[]) {
    ...
    denseB_values_h = (double *)malloc(ld * ncols * sizeof(double));
    ...
    CUDA_CALL_AND_CHECK(cudaMalloc(&denseB_values_d, ld * ncols * sizeof(double)),
                        "cudaMalloc for denseB values");
    CUDA_CALL_AND_CHECK(cudaMalloc(&denseB_values_d2, ld * ncols * sizeof(double)),
                        "cudaMalloc for future denseB values");

    /* Copy host memory to device for B */
    CUDA_CALL_AND_CHECK(cudaMemcpy(denseB_values_d, denseB_values_h,
                                   ld * ncols * sizeof(double), cudaMemcpyHostToDevice),
                        "cudaMemcpy H2D for denseB_values");
    ...
    CUDSS_CALL_AND_CHECK(cudssMatrixCreateDn(&B, nrows, ncols, ld, denseB_values_d,
    ...
    cudssDataType_t denseB_type_out;
    cudssLayout_t   denseB_layout_out;
    CUDSS_CALL_AND_CHECK(cudssMatrixGetDn(B, &nrows_out, &ncols_out, &ldb_out,
                                          &denseB_values_out, &denseB_type_out,
                                          &denseB_layout_out),
                         status, "cudssMatrixGetDn for B");
    if (denseB_values_out != denseB_values_d || nrows_out != nrows ||
        ncols_out != ncols || ldb_out != ld || denseB_type_out != CUDSS_R_64F ||
    ...
    CUDSS_CALL_AND_CHECK(
        cudssMatrixGetDn(B, NULL, NULL, NULL, &denseB_values_out, NULL, NULL), status,
        "cudssMatrixGetDn for B");
    CUDA_CALL_AND_CHECK(cudaMemcpy(denseB_values_h, denseB_values_out,
                                   ld * ncols * sizeof(double), cudaMemcpyDeviceToHost),
                        "cudaMemcpy D2H for denseB_values");
    ...
    CUDSS_CALL_AND_CHECK(cudssMatrixSetValues(B, denseB_values_d2), status,
