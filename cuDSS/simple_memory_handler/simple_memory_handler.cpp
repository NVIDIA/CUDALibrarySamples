...
/*
 * This is just a working example of a simple C++ memory pool solely for illustration
 * purposes only.
 */
class ExampleDeviceMemPool {
  public:
    int alloc(void **ptr, size_t size, cudaStream_t stream) {
        ...
        printf("alloc() from the ExampleDeviceMemPool allocates memory at %p (allocation "
               "size %zu)\n",
               (void *)*ptr, size);
        return status;
    }
...
int main(int argc, char *argv[]) {
    ...
    ExampleDeviceMemPool pool =
        ExampleDeviceMemPool(); // kept alive for the entire process in real apps
    ...
    /* Allocate device memory for A, x and b
       Note: These device memory allocations are "external" to cuDSS and hence can be
       regular cudaMalloc() calls, or use the memory pool defined above, it does not
       matter. In real applications one should use the memory pool alloc/dealloc routines
       but here, to keep example code short, we rely on regular cudaMalloc/cudaFree(). */
