...
static void print_sparse_matrix(int nrows, int *csr_offsets, int *csr_columns,
                                double *csr_values) {
...
int main(int argc, char *argv[]) {
    ...
    i                    = 0;
    csr_values_h[0][i++] = 4.0;
    csr_values_h[0][i++] = 1.0;
    csr_values_h[0][i++] = 3.0;
    csr_values_h[0][i++] = 2.0;
    csr_values_h[0][i++] = 5.0;
    csr_values_h[0][i++] = 1.0;
    csr_values_h[0][i++] = 1.0;
    csr_values_h[0][i++] = 2.0;
    ...
    i                    = 0;
    csr_values_h[1][i++] = 3.0;
    csr_values_h[1][i++] = 1.0;
    csr_values_h[1][i++] = 2.0;
    csr_values_h[1][i++] = 1.0;
    csr_values_h[1][i++] = 6.0;
    csr_values_h[1][i++] = 2.0;
    csr_values_h[1][i++] = 2.0;
    csr_values_h[1][i++] = 5.0;
    csr_values_h[1][i++] = 7.0;
    csr_values_h[1][i++] = 3.0;
    ...
    for (int i = 0; i < batchCount; i++) {
        ...
        CUDA_CALL_AND_CHECK(
            cudaMalloc(&batch_b_values_h[i], nrhs[i] * n[i] * sizeof(double)),
            "cudaMalloc for b_values");

        CUDA_CALL_AND_CHECK(
            cudaMalloc(&batch_csr_offsets_h2[i], (n[i] + 1) * sizeof(int)),
        ...
        CUDA_CALL_AND_CHECK(
            cudaMalloc(&batch_b_values_h2[i], nrhs[i] * n[i] * sizeof(double)),
            "cudaMalloc for b2_values");

        /* Copy host memory to device for A and b */
        CUDA_CALL_AND_CHECK(cudaMemcpy(batch_csr_offsets_h[i], csr_offsets_h[i],
                                       (n[i] + 1) * sizeof(int), cudaMemcpyHostToDevice),
                            "cudaMemcpy for csr_offsets");
        CUDA_CALL_AND_CHECK(cudaMemcpy(batch_csr_columns_h[i], csr_columns_h[i],
                                       nnz[i] * sizeof(int), cudaMemcpyHostToDevice),
                            "cudaMemcpy for csr_columns");
        CUDA_CALL_AND_CHECK(cudaMemcpy(batch_csr_values_h[i], csr_values_h[i],
                                       nnz[i] * sizeof(double), cudaMemcpyHostToDevice),
                            "cudaMemcpy for csr_values");
        CUDA_CALL_AND_CHECK(cudaMemcpy(batch_b_values_h[i], b_values_h[i],
                                       nrhs[i] * n[i] * sizeof(double),
                                       cudaMemcpyHostToDevice),
                            "cudaMemcpy for b_values");
        CUDA_CALL_AND_CHECK(cudaMemcpy(batch_csr_values_h2[i], csr_values_h[i],
                                       nnz[i] * sizeof(double), cudaMemcpyHostToDevice),
                            "cudaMemcpy for csr_values2");
        CUDA_CALL_AND_CHECK(cudaMemcpy(batch_b_values_h2[i], b_values_h[i],
                                       nrhs[i] * n[i] * sizeof(double),
                                       cudaMemcpyHostToDevice),
                            "cudaMemcpy for batch_b_values_h2");
    }
    ...
    CUDA_CALL_AND_CHECK(cudaMemcpy(batch_csr_offsets_d, batch_csr_offsets_h,
                                   batchCount * sizeof(int *), cudaMemcpyHostToDevice),
                        "cudaMemcpy for batch_csr_offsets");
    CUDA_CALL_AND_CHECK(cudaMemcpy(batch_csr_columns_d, batch_csr_columns_h,
                                   batchCount * sizeof(int *), cudaMemcpyHostToDevice),
                        "cudaMemcpy for csr_columns");
    CUDA_CALL_AND_CHECK(cudaMemcpy(batch_csr_values_d, batch_csr_values_h,
                                   batchCount * sizeof(double *), cudaMemcpyHostToDevice),
                        "cudaMemcpy for batch_csr_values");
    CUDA_CALL_AND_CHECK(cudaMemcpy(batch_b_values_d, batch_b_values_h,
                                   batchCount * sizeof(double *), cudaMemcpyHostToDevice),
                        "cudaMemcpy for batch_b_values_d");
    CUDA_CALL_AND_CHECK(cudaMemcpy(batch_csr_values_d2, batch_csr_values_h2,
                                   batchCount * sizeof(double *), cudaMemcpyHostToDevice),
                        "cudaMemcpy for batch_csr_values2");
    CUDA_CALL_AND_CHECK(cudaMemcpy(batch_b_values_d2, batch_b_values_h2,
                                   batchCount * sizeof(double *), cudaMemcpyHostToDevice),
                        "cudaMemcpy for batch_b_values_d2");
    ...
    /* Create matrix objects for the right-hand side b and solution x (as batches of dense
     * matrices). */
    cudssMatrix_t b;

    int *nrows = n, *ncols = n;
    int *ldb = nrows;
    CUDSS_CALL_AND_CHECK(cudssMatrixCreateBatchDn(&b, batchCount, nrows, nrhs, ldb,
                                                  batch_b_values_d, CUDSS_R_32I,
    ...
    CUDSS_CALL_AND_CHECK(cudssMatrixCreateBatchCsr(
                             &A, batchCount, nrows, ncols, nnz, batch_csr_offsets_d, NULL,
                             batch_csr_columns_d, batch_csr_values_d, CUDSS_R_32I,
    ...
    cudssDataType_t       offset_type_out, index_type_out, value_type_out;
    cudssMatrixType_t     mtype_out;
    cudssMatrixViewType_t mview_out;
    cudssIndexBase_t      index_base_out;
    CUDSS_CALL_AND_CHECK(
        cudssMatrixGetBatchCsr(A, &batchCount_out, &nrows_out, &ncols_out, &nnz_out,
                               &batch_csr_offsets_out, NULL, &batch_csr_columns_out,
                               &batch_csr_values_out, &offset_type_out, &index_type_out,
                               &value_type_out, &mtype_out, &mview_out, &index_base_out),
        status, "cudssMatrixGetBatchCsr for A");

    if (batch_csr_offsets_out != batch_csr_offsets_d ||
        batch_csr_columns_out != batch_csr_columns_d ||
        batch_csr_values_out != batch_csr_values_d || batchCount_out != batchCount ||
        nrows_out != nrows || ncols_out != ncols || nnz_out != nnz ||
        index_type_out != CUDSS_R_32I || value_type_out != CUDSS_R_64F ||
    ...
    CUDSS_CALL_AND_CHECK(cudssMatrixGetBatchDn(b, &batchCount_out, &nrows_out, &nrhs_out,
                                               &ldb_out, &batch_b_values_out,
                                               &index_type_out, &value_type_out,
                                               &layout_out),
                         status, "cudssMatrixGetBatchDn for B");

    if (batch_b_values_out != batch_b_values_d || batchCount_out != batchCount ||
    ...
    CUDSS_CALL_AND_CHECK(cudssMatrixSetBatchValues(A, batch_csr_values_d2), status,
                         "cudssMatrixSetBatchValues for A");

    CUDSS_CALL_AND_CHECK(cudssMatrixSetBatchValues(b, batch_b_values_d2), status,
                         "cudssMatrixSetBatchValues for B");

    CUDSS_CALL_AND_CHECK(cudssMatrixGetBatchCsr(A, NULL, NULL, NULL, NULL, NULL, NULL,
                                                NULL, &batch_csr_values_out, NULL, NULL,
                                                NULL, NULL, NULL, NULL),
                         status, "cudssMatrixGetBatchCsr for A");

    CUDSS_CALL_AND_CHECK(cudssMatrixGetBatchDn(b, NULL, NULL, NULL, NULL,
                                               &batch_b_values_out, NULL, NULL, NULL),
                         status, "cudssMatrixGetBatchDn for B");
    ...
    if (out_csr_value[0] != new_csr_value[0] || out_csr_value[1] != new_csr_value[1]) {
        printf("Error: Example FAILED: modification of A values out-of-place failed\n"
               "value of matrix A in each batch equals %3.3e and %3.3e (expected %3.3e "
               "%3.3e)\n",
    ...
    if (out_b_value[0] != new_b_value[0] || out_b_value[1] != new_b_value[1]) {
        printf("Error: Example FAILED: modification of B values out-of-place failed\n"
               "value of matrix B in each batch equals %3.3e and %3.3e (expected %3.3e "
               "%3.3e)\n",
    ...
    i                      = 0;
    csr_columns_h2[0][i++] = 0;
    csr_columns_h2[0][i++] = 2;
    csr_columns_h2[0][i++] = 1;
    csr_columns_h2[0][i++] = 2;
    csr_columns_h2[0][i++] = 2;
    ...
    i                     = 0;
    csr_values_h2[0][i++] = 4.0;
    csr_values_h2[0][i++] = 1.0;
    csr_values_h2[0][i++] = 3.0;
    ...
    i                      = 0;
    csr_columns_h2[1][i++] = 0;
    csr_columns_h2[1][i++] = 5;
    csr_columns_h2[1][i++] = 1;
    csr_columns_h2[1][i++] = 4;
    csr_columns_h2[1][i++] = 2;
    csr_columns_h2[1][i++] = 4;
    ...
    i                     = 0;
    csr_values_h2[1][i++] = 3.0;
    csr_values_h2[1][i++] = 1.0;
    csr_values_h2[1][i++] = 2.0;
    csr_values_h2[1][i++] = 1.0;
    csr_values_h2[1][i++] = 6.0;
    csr_values_h2[1][i++] = 2.0;
    csr_values_h2[1][i++] = 5.0;
    csr_values_h2[1][i++] = 7.0;
    csr_values_h2[1][i++] = 3.0;
    ...
    CUDA_CALL_AND_CHECK(cudaMemcpy(batch_csr_offsets_d2, batch_csr_offsets_h2,
                                   batchCount * sizeof(int *), cudaMemcpyHostToDevice),
                        "cudaMemcpy for batch_csr_offsets2");
    CUDA_CALL_AND_CHECK(cudaMemcpy(batch_csr_columns_d2, batch_csr_columns_h2,
                                   batchCount * sizeof(int *), cudaMemcpyHostToDevice),
                        "cudaMemcpy for csr_columns2");
    CUDA_CALL_AND_CHECK(cudaMemcpy(batch_csr_values_d2, batch_csr_values_h2,
                                   batchCount * sizeof(double *), cudaMemcpyHostToDevice),
    ...
    CUDSS_CALL_AND_CHECK(cudssMatrixSetBatchCsrPointers(A, batch_csr_offsets_d2, NULL,
                                                        batch_csr_columns_d2,
                                                        batch_csr_values_d2),
                         status, "cudssMatrixSetBatchCsrPointers for A");
    int *new_nnz_out;
    CUDSS_CALL_AND_CHECK(
        cudssMatrixGetBatchCsr(A, NULL, NULL, NULL, (void **)&new_nnz_out,
                               &batch_csr_offsets_out, NULL, &batch_csr_columns_out,
                               &batch_csr_values_out, NULL, NULL, NULL, NULL, NULL, NULL),
        status, "cudssMatrixGetBatchCsr for A");
    ...
    if (new_nnz_out[0] == nnz[0] && new_nnz_out[1] == nnz[1]) {
        printf("Note: Changing the matrix structure does not change the nnz returned by\n"
               "cudssMatrixGetBatchCsr. The original value = %d and %d passed to "
               "cudssMatrixCreateBatchCsr are\n"
               "returned\n",
        ...
        printf("Error: Example FAILED: unexpected nnz = %d and %d are returned after "
               "changing the\n"
    ...
    int format;
    CUDSS_CALL_AND_CHECK(cudssMatrixGetFormat(A, &format), status,
                         "cudssMatrixGetFormat for A");
    ...
    CUDSS_CALL_AND_CHECK(cudssMatrixGetFormat(b, &format), status,
                         "cudssMatrixGetFormat for b");
