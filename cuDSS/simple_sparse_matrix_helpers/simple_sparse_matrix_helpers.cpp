...
void print_sparse_matrix(int nrows, int *csr_offsets, int *csr_columns,
                         double *csr_values) {
...
int main(int argc, char *argv[]) {
    ...
    if (!csrA_offsets_h || !csrA_columns_h || !csrA_values_h || !csrA_offsets_h2 ||
        !csrA_columns_h2 || !csrA_values_h2) {
    ...
    CUDSS_CALL_AND_CHECK(cudssMatrixCreateCsr(&A, nrows, ncols, nnz, csrA_offsets_d, NULL,
                                              csrA_columns_d, csrA_values_d, CUDSS_R_32I,
    ...
    CUDSS_CALL_AND_CHECK(cudssMatrixGetCsr(A, &nrows_out, &ncols_out, &nnz_out,
                                           &csrA_offsets_out, NULL, &csrA_columns_out,
                                           &csrA_values_out, &offset_type_out,
                                           &index_type_out, &value_type_out, &mtype_out,
                                           &mview_out, &index_base_out),
                         status, "cudssMatrixGetCsr for A");

    if (csrA_offsets_out != csrA_offsets_d || csrA_columns_out != csrA_columns_d ||
        csrA_values_out != csrA_values_d || nrows_out != nrows || ncols_out != ncols ||
        nnz_out != nnz || offset_type_out != CUDSS_R_32I ||
    ...
    CUDA_CALL_AND_CHECK(
        cudaMemcpy(csrA_values_d, &new_value, 1 * sizeof(double), cudaMemcpyHostToDevice),
        "cudaMemset");
    CUDSS_CALL_AND_CHECK(cudssMatrixGetCsr(A, NULL, NULL, NULL, NULL, NULL, NULL,
                                           &csrA_values_out, NULL, NULL, NULL, NULL, NULL,
                                           NULL),
                         status, "cudssMatrixGetCsr for A");
    CUDA_CALL_AND_CHECK(cudaMemcpy(csrA_offsets_h, csrA_offsets_out,
                                   (nrows + 1) * sizeof(int), cudaMemcpyDeviceToHost),
                        "cudaMemcpy DTH for csrA_offsets");
    CUDA_CALL_AND_CHECK(cudaMemcpy(csrA_columns_h, csrA_columns_out, nnz * sizeof(int),
                                   cudaMemcpyDeviceToHost),
                        "cudaMemcpy DTH for csrA_columns");
    CUDA_CALL_AND_CHECK(cudaMemcpy(csrA_values_h, csrA_values_out, nnz * sizeof(double),
                                   cudaMemcpyDeviceToHost),
                        "cudaMemcpy DTH for csrA_values");
    ...
    CUDSS_CALL_AND_CHECK(cudssMatrixSetValues(A, csrA_values_d2), status,
                         "cudssMatrixSetValues for A");
    CUDSS_CALL_AND_CHECK(cudssMatrixGetCsr(A, NULL, NULL, NULL, NULL, NULL, NULL,
                                           &csrA_values_out, NULL, NULL, NULL, NULL, NULL,
                                           NULL),
                         status, "cudssMatrixGetCsr for A");
    ...
    CUDA_CALL_AND_CHECK(cudaMemcpy(csrA_offsets_d2, csrA_offsets_h2,
                                   (nrows + 1) * sizeof(int), cudaMemcpyHostToDevice),
                        "cudaMemcpy H2D for csrA_offsets");
    ...
    int64_t new_nnz_out;
    CUDSS_CALL_AND_CHECK(cudssMatrixGetCsr(A, NULL, NULL, &new_nnz_out, &csrA_offsets_out,
                                           NULL, &csrA_columns_out, &csrA_values_out,
    ...
    if (new_nnz_out == nnz_out) {
        printf("Note: Changing the matrix structure does not change the nnz returned by\n"
               "cudssMatrixGetCsr. The original value = %ld passed to "
               "cudssMatrixCreateCsr is\n"
               "returned\n",
               new_nnz_out);
        ...
        printf(
            "Error: Example FAILED: unexpected nnz = %ld is returned after changing the\n"
